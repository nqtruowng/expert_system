...
try:
    ...
    print("Missing dependency: beautifulsoup4. Please install it with: pip install beautifulsoup4")
    sys.exit(1)
from difflib import *
def select():
...
def countrySearch(country, key):
    ...
    # Special fuzzy matches: ";matches <keyword>"
    if key.startswith(";matches"):
        pattern = key.split(" ", 1)[1].strip() if " " in key else ""
        keys = list(possibilities.keys())
        results = get_close_matches(pattern, keys) if pattern else keys[:20]
        for result in results:
            print(result)
        return

    results = get_close_matches(key, list(possibilities.keys()))
...
def parseHtml(htmlFile):
	...
	for attr in soup.find_all("div", class_="category"):
		aTag = attr.find("a")
		if aTag:
			trTag = attr.parent.parent.find_next_sibling("tr")

			if len(trTag) > 1:
				contents = []
				for div in trTag.find_all("div", class_="category"):
				...
				for div in trTag.find_all("div", class_="category_data"):
					if div.string:
						contents.append(div.string)

				data = "\r\n".join(contents)
			elif trTag.find("td"):
				data = trTag.find("td").find("div", class_="category_data").get_text(strip=True)
				print(data)
			else:
				data = trTag.find("div", class_="category_data").get_text(strip=True)
